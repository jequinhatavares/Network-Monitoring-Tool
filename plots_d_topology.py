...
def get_dfs():
    runs = []
    for file in ["logs/distributed_nn_12_strategy_topology/" + f for f in os.listdir(
            "logs/distributed_nn_12_strategy_topology/") if f.startswith('run-app-init')]:
    ...
    runs = []
    for file in ["logs/distributed_nn_12_strategy_topology/" + f for f in os.listdir(
            "logs/distributed_nn_12_strategy_topology/") if f.startswith('run-app-inference')]:
        with open(file, 'r') as f:
            runs += json.load(f)
    ...
    runs = []
    with open('logs/distributed_nn_12_strategy_topology/run-continuous-messages-2.json', 'r') as f:
        runs += json.load(f)
...
def plot_scatter_inference_time(df: pd.DataFrame):
    ...
    fig2.write_image("images/inference_time_d_nn_12_topology.png", scale=3)
...
def create_throughput_bar_plot(df, metrics):
    ...
    fig.write_image("images/throughput_d_nn_12_topology.png", scale=3)
...
def create_four_category_pie(df):
    ...
    fig.write_image("images/messages_pie_d_nn_12_topology.png",
